...
engine_output <- function (options, code, out, extra = NULL) {
  if (missing(code) && is.list(out)) return(unlist(knitr::sew(out, options)))
  if (!is.logical(options$echo)) code = code[options$echo]
  if (length(code) != 1) code = single_string(code)
  if (options$engine == "sas" && length(out) > 1 && !grepl("[[:alnum:]]", out[2])) out = utils::tail(out, -3)
  if (length(out) != 1) out = single_string(out)
  out = sub("([^\n]+)$", "\\1\n", out)
